...
![sheme2](./img/shem-of-diplom2.jpg)  
...
- PHP-FPM  
- NGINX для проксирования прометеус и графана
...
- python3-cerbot бот для создания и продления сертификата от Let`s Encrypt  
  Сервер взаимодействует по внутреней сети 10.0.0.0/24 с **_mysqlservernode1_** - раположена БД на MySQL, с **_drbdnode1_** сетевое хранилище для файлов сайта, смонтирована как /var/www/html (корневая папка сайта) через NFS клиент и с **_backupserver_** для хранения файлов конфигурации выше описанных сервисов.
...
5. **_backupserver_**  Cистема резервного копирования Borg Backup. Взаимодействует с **_frontserver_**, **_mysqlservernode2_**, **_drbdnode1_** . Будут хранится по 24 архива с каждого сервера - каждые три часа за последние трое суток.
